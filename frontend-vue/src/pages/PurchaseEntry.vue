<template>
  <div class="min-h-screen bg-gray-100">
      ...
      <div class="max-w-full mx-auto px-2 sm:px-4 py-2 sm:py-3">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0">
          <div class="flex items-center space-x-2 sm:space-x-4">
            <ShoppingBag class="w-5 h-5 sm:w-6 sm:h-6 text-amber-600" />
            <h1 class="text-lg sm:text-xl lg:text-2xl font-semibold text-gray-800"><span class="hidden sm:inline">Purchase Entry</span><span class="sm:hidden">Purchase</span></h1>
          </div>
    ...
    <div class="bg-white mx-2 sm:mx-4 mt-2 sm:mt-4 border border-gray-300 rounded shadow-sm">
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-2 sm:gap-4 p-2 sm:p-4 border-b border-gray-300 bg-gray-50">
        ...
        <div class="text-center"><div class="text-xs text-gray-600 mb-1 font-medium">VAT 15%</div><div class="text-base sm:text-lg font-bold text-blue-600">${{ formatNum(totalVat) }}</div></div>
        <div class="text-center border-l-0 sm:border-l-2 border-gray-400 sm:pl-4"><div class="text-xs text-gray-600 mb-1 font-medium">Subtotal</div><div class="text-xl sm:text-2xl font-bold text-green-600">${{ formatNum(totalSubtotal) }}</div></div>
      </div>
    </div>
    <div class="bg-white mx-2 sm:mx-4 my-2 sm:my-4 border border-gray-300 rounded shadow-sm overflow-hidden">
        ...
        <table class="min-w-full border-collapse text-xs sm:text-sm" style="font-family: Calibri, Arial, sans-serif">
          <thead class="sticky top-0 z-10">
            <tr class="bg-gray-50 border-b-2 border-gray-400">
              ...
              <th class="border border-gray-300 bg-gray-100 text-center font-semibold text-gray-700 px-1 sm:px-2 py-1.5 sm:py-2" style="min-width: 85px">Price</th>
              <th class="border border-gray-300 bg-gray-100 text-center font-semibold text-gray-700 px-1 sm:px-2 py-1.5 sm:py-2" style="min-width: 65px">Qty</th>
              ...
              <th class="border border-gray-300 bg-gray-100 text-center font-semibold text-gray-700 px-1 sm:px-2 py-1.5 sm:py-2" style="min-width: 85px">VAT 15%</th>
              <th class="border border-gray-300 bg-gray-100 text-center font-semibold text-gray-700 px-1 sm:px-2 py-1.5 sm:py-2" style="min-width: 95px">Subtotal</th>
          ...
          <tbody>
            <tr v-for="(row, rowIndex) in rows" :key="row.id" class="hover:bg-blue-50">
              ...
              <td class="border border-gray-300 px-0.5 sm:px-1 py-0"><input v-model.number="row.price" type="number" min="0" step="0.01" placeholder="0" class="w-full px-1 sm:px-2 py-1.5 sm:py-1 text-xs sm:text-sm text-right border-0 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[36px] sm:min-h-[28px]" /></td>
              <td class="border border-gray-300 px-0.5 sm:px-1 py-0"><input v-model.number="row.qty" type="number" min="0" step="1" placeholder="0" class="w-full px-1 sm:px-2 py-1.5 sm:py-1 text-xs sm:text-sm text-right border-0 focus:outline-none focus:ring-2 focus:ring-blue-500 min-h-[36px] sm:min-h-[28px]" /></td>
              <td class="border border-gray-300 px-1 sm:px-2 py-1.5 sm:py-1 text-right text-gray-700 tabular-nums text-xs sm:text-sm">{{ formatNum(lineAmount(row)) }}</td>
              <td class="border border-gray-300 px-1 sm:px-2 py-1.5 sm:py-1 text-right text-gray-700 tabular-nums text-xs sm:text-sm">{{ formatNum(lineVat(row)) }}</td>
              <td class="border border-gray-300 px-1 sm:px-2 py-1.5 sm:py-1 text-right text-gray-700 tabular-nums text-xs sm:text-sm font-medium">{{ formatNum(lineSubtotal(row)) }}</td>
      ...
      <div v-if="rows.length === 0" class="text-center py-12 text-gray-500"><ShoppingBag class="w-12 h-12 mx-auto mb-4 text-gray-400" /><p class="text-sm">No rows yet. Add a row to enter purchases.</p></div>
    </div>
  </div>
</template>
...
function newRow(overrides = {}) {
  ...
  return { id: nextId++, date: today, customerCode: '', customerName: '', invoiceNumber: '', productName: '', price: 0, qty: 0, ...overrides }
}

function addRow() { rows.value.push(newRow()) }
function removeRow(index) { rows.value.splice(index, 1) }

function lineAmount(row) { return (Number(row.price) || 0) * (Number(row.qty) || 0) }
function lineVat(row) { return lineAmount(row) * VAT_RATE }
function lineSubtotal(row) { return lineAmount(row) + lineVat(row) }
...
const totalAmount = computed(() => rows.value.reduce((sum, r) => sum + lineAmount(r), 0))
const totalVat = computed(() => rows.value.reduce((sum, r) => sum + lineVat(r), 0))
const totalSubtotal = computed(() => rows.value.reduce((sum, r) => sum + lineSubtotal(r), 0))
