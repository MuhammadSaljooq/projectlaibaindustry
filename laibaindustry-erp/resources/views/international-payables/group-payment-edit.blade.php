...
<html lang="en">
<head>
@include('partials.frontend-head', ['title' => 'Edit combined international payable payment - ERP'])
...
<a href="{{ route('international-payables.group', ['groupKey' => $groupKeyEncoded]) }}" class="st-btn-secondary h-9 px-3 inline-flex items-center gap-2 text-[10px]">
<span class="material-symbols-outlined text-[18px]">arrow_back</span>
<span class="hidden sm:inline">Vendor invoices</span>
</a>
</div>
</header>
...
<p class="text-[10px] font-bold uppercase tracking-[0.2em] text-[#586064]">INT_AP_COMBINED_PAYMENT</p>
...
<p class="st-label mb-4">Update amount/date/notes</p>
...
<form method="post" action="{{ route('international-payables.group.payments.update', ['groupKey' => $groupKeyEncoded, 'internationalPayableGroupPayment' => $groupPayment]) }}" class="flex flex-col gap-6">
@csrf
@method('PATCH')
<div class="flex flex-col gap-1">
...
<div class="flex flex-col gap-1">
<label class="st-label" for="notes">Notes (optional)</label>
<input class="st-input text-sm max-w-md" type="text" id="notes" name="notes" maxlength="500" value="{{ old('notes', $groupPayment->notes) }}">
</div>
