...
<span class="text-xl font-bold text-slate-800 dark:text-white hidden sm:inline">Edit Expense</span>
</a>
</div>
</header>
...
<div class="sm:hidden">
<h2 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Expense</h2>
</div>
...
<input type="text" name="type" id="type" value="{{ old('type', $expense->type) }}" placeholder="e.g. Rent, Utilities" required
...
<input type="number" name="amount" id="amount" value="{{ old('amount', $expense->amount) }}" step="0.01" min="0" placeholder="0.00" required
    class="w-full h-10 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-4 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-primary focus:border-primary">
</div>
</div>
<div class="flex flex-wrap gap-3 mt-6">
<button type="submit" class="h-10 px-5 bg-primary hover:bg-blue-600 text-white text-sm font-bold rounded-lg transition-colors whitespace-nowrap">Update Expense</button>
