...
<a href="{{ route('international-purchases.show', $order) }}" class="st-btn-secondary h-9 px-3 inline-flex items-center gap-2 text-[10px]">
...
<p class="text-sm text-[#586064] mt-2 font-mono">{{ $order->invoice_number ?: '#'.$order->id }}</p>
</div>
...
<form method="POST" action="{{ route('international-purchases.update', $order) }}" id="intl-purchase-form" novalidate>
@csrf
@method('PUT')
...
<option value="{{ $s->id }}" @selected((string) old('supplier_id', $order->supplier_id) === (string) $s->id)>{{ $s->name }}</option>
@endforeach
</select>
...
<input class="st-input w-full h-10 px-3 text-sm" type="date" name="date" id="date" value="{{ old('date', $order->date->format('Y-m-d')) }}" required>
</div>
<div>
<label class="st-label block mb-2" for="invoice_number">Invoice / reference #</label>
<input class="st-input w-full h-10 px-3 text-sm font-mono" type="text" name="invoice_number" id="invoice_number" value="{{ old('invoice_number', $order->invoice_number) }}" maxlength="191" placeholder="Optional">
</div>
</div>
...
<a href="{{ route('international-purchases.show', $order) }}" class="st-btn-secondary h-10 px-5 inline-flex items-center gap-2">Cancel</a>
</div>
</form>
</div>
...
<p class="text-xs text-[#586064]">Remove this invoice and all lines permanently.</p>
</div>
<form method="POST" action="{{ route('international-purchases.destroy', $order) }}" data-confirm-delete="{{ e('Delete this invoice and all lines?') }}">
@csrf
@method('DELETE')
