...
<p class="text-sm text-[#586064] mt-2">{{ $internationalPurchase->product_name }} · {{ format_display_date($internationalPurchase->date) }}</p>
</div>
...
<option value="{{ $s->id }}" @selected((string) old('supplier_id', $internationalPurchase->supplier_id) === (string) $s->id)>{{ $s->name }}</option>
@endforeach
</select>
