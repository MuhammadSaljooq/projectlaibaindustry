...
<html lang="en">
<head>
@include('partials.frontend-head', ['title' => 'Edit Stock Addition - Laiba Safety'])
...
<form method="POST" action="{{ route('stock-additions.destroy', $addition) }}" onsubmit="return confirm('Delete this stock addition? This will reverse the inventory change.')">
@csrf
@method('DELETE')
<button type="submit" class="h-10 px-4 inline-flex items-center gap-2 border border-[#9F403D] text-[#9F403D] text-sm hover:bg-[#9F403D] hover:text-white transition-colors whitespace-nowrap">
...
<p class="text-[10px] font-bold uppercase tracking-[0.2em] text-[#586064]">STOCK_EDIT_{{ $addition->id }}</p>
<h1 class="text-3xl md:text-4xl font-black uppercase tracking-tighter text-[#2B3437] leading-none">Edit Stock Addition</h1>
</div>
...
<div class="border border-[#ABB3B7] p-4 bg-[#EAEFF1] text-sm text-[#586064]">
<p><span class="font-bold text-[#2B3437]">Note:</span> Changing the quantity or product will automatically adjust the inventory to reflect the difference.</p>
</div>

<div class="st-paper border border-[#ABB3B7] p-6 md:p-8 bg-white">
<form method="POST" action="{{ route('stock-additions.update', $addition) }}">
@csrf
@method('PUT')
...
<option value="{{ $p->id }}" {{ (string) old('product_id', $addition->product_id) === (string) $p->id ? 'selected' : '' }}>
...
<input class="st-input w-full h-10 px-3 text-sm @error('date') !border-[#9F403D] @enderror" type="date" name="date" id="date" value="{{ old('date', $addition->date?->format('Y-m-d')) }}" required>
...
<input class="st-input w-full h-10 px-3 text-sm tabular-nums @error('quantity') !border-[#9F403D] @enderror" type="number" name="quantity" id="quantity" value="{{ old('quantity', $addition->quantity) }}" min="1" step="1" placeholder="0" required>
...
<input class="st-input w-full h-10 px-3 text-sm tabular-nums @error('unit_cost') !border-[#9F403D] @enderror" type="number" name="unit_cost" id="unit_cost" value="{{ old('unit_cost', $addition->unit_cost) }}" min="0" step="0.01" placeholder="0.00">
...
<input class="st-input w-full h-10 px-3 text-sm @error('reference') !border-[#9F403D] @enderror" type="text" name="reference" id="reference" value="{{ old('reference', $addition->reference) }}" maxlength="255" placeholder="e.g. INV-2026-001">
...
<textarea class="st-input w-full px-3 py-2 text-sm resize-none @error('notes') !border-[#9F403D] @enderror" name="notes" id="notes" rows="3" maxlength="1000" placeholder="Any additional notes…">{{ old('notes', $addition->notes) }}</textarea>
