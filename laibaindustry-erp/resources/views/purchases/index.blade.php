...
<ul class="list-disc list-inside text-[13px]">@foreach ($errors->all() as $err)<li>{{ $err }}</li>@endforeach</ul>
</div>
@endif
...
<tr class="st-tr cursor-pointer focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-[#5E5E5E]" data-purchase-group-url="{{ $groupUrl }}" role="link" tabindex="0" aria-label="Open purchases for {{ e($group['display_name']) }}">
...
@empty
<tr><td colspan="8" class="px-6 py-14 text-center text-sm text-[#586064] border-b border-[#ABB3B7]">No purchases yet. @if(auth()->user()->role !== 'viewer')<a href="{{ route('purchases.create') }}" class="text-[#5E5E5E] font-bold underline underline-offset-2">Create your first purchase</a>@endif</td></tr>
@endforelse
</tbody>
</table>
</div>
...
Showing <span class="font-bold text-[#2B3437] tabular-nums">{{ $purchaseGroups->count() }}</span> customer groups · <span class="font-bold text-[#2B3437] tabular-nums">{{ $totalInvoiceCount }}</span> invoices total
@else
No results
@endif
</p>
...
<script>
(function () {
    document.querySelectorAll('tr[data-purchase-group-url]').forEach(function (row) {
