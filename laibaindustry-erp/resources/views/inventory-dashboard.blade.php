...
<div class="bg-white dark:bg-black rounded-xl border border-slate-200 dark:border-slate-700 p-5 shadow-sm hover:shadow-md transition-shadow">
...
<div class="p-2 bg-gray-100 dark:bg-gray-800 text-gray-800 dark:text-gray-200 rounded-lg">
...
<div class="bg-white dark:bg-black rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm flex flex-col flex-1 min-h-[500px]">
<form method="GET" action="{{ route('inventory.dashboard') }}" class="border-b border-slate-200 dark:border-slate-700">
<div class="p-5 flex flex-col gap-4">
<div class="flex flex-wrap items-center justify-between gap-3">
<h3 class="text-base font-semibold text-slate-800 dark:text-white">Inventory</h3>
@if(auth()->user()->role !== 'viewer')
<a href="{{ route('products.create') }}" class="h-10 px-4 bg-black hover:bg-gray-800 text-white text-sm font-bold rounded-lg inline-flex items-center justify-center gap-2 whitespace-nowrap transition-colors shadow-sm hover:shadow active:scale-95 shrink-0">
...
<div class="relative flex-1 min-w-0 w-full sm:max-w-[240px]">
...
<input class="h-10 w-full min-w-0 pl-10 pr-4 text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary placeholder-slate-400 text-slate-700 dark:text-slate-200 outline-none" name="search" type="text" placeholder="Search inventory..." value="{{ request('search') }}"/>
</div>
<div class="relative w-full sm:w-[180px] min-w-0">
<select class="h-10 pl-3 pr-9 w-full text-sm bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-slate-700 dark:text-slate-200 outline-none appearance-none cursor-pointer" name="category_id">
...
<a href="{{ route('inventory.dashboard') }}" class="h-10 px-4 inline-flex items-center justify-center gap-2 text-sm font-medium text-slate-600 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg border border-slate-200 dark:border-slate-600 whitespace-nowrap shrink-0">Clear</a>
</div>
</div>
</div>
</div>
...
<div>
<p class="text-sm font-semibold text-slate-900 dark:text-white group-hover:text-black dark:group-hover:text-white transition-colors">{{ $product->name }}</p>
...
@else
<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-200 border border-gray-300 dark:border-gray-600"><span class="w-1.5 h-1.5 rounded-full bg-gray-600 mr-1.5"></span>In Stock</span>
@endif
</td>
<td class="px-6 py-4 text-right">
@if(auth()->user()->role !== 'viewer')
<div class="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
<a href="{{ route('products.edit', $product) }}" class="p-1.5 text-slate-400 hover:text-black dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-700 rounded-md transition-colors"><span class="material-symbols-outlined text-[20px]">edit</span></a>
...
No products yet. @if(auth()->user()->role !== 'viewer')<a href="{{ route('products.create') }}" class="text-black dark:text-white font-medium hover:underline">Add your first product</a>@else<span>No products recorded yet.</span>@endif
</td>
</tr>
@endforelse
</tbody>
