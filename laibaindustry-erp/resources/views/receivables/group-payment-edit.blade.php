...
<html lang="en">
<head>
@include('partials.frontend-head', ['title' => 'Edit combined payment - ERP'])
...
<a href="{{ route('receivables.group', ['groupKey' => $groupKeyEncoded]) }}" class="st-btn-secondary h-9 px-3 inline-flex items-center gap-2 text-[10px]">
...
<p class="text-[10px] font-bold uppercase tracking-[0.2em] text-[#586064]">AR_COMBINED_PAYMENT</p>
...
<form method="post" action="{{ route('receivables.group.payments.update', ['groupKey' => $groupKeyEncoded, 'receivableGroupPayment' => $groupPayment]) }}" class="flex flex-col gap-6">
@csrf
@method('PATCH')
<div class="flex flex-col gap-1">
