...
<h2 class="text-xl font-bold text-slate-800 dark:text-white hidden sm:block">Edit Sale</h2>
</div>
</header>
<div class="flex-1 overflow-y-auto p-6 scroll-smooth">
<div class="max-w-[1400px] mx-auto flex flex-col gap-6">
<div class="sm:hidden">
<h2 class="text-2xl font-bold text-slate-800 dark:text-white">Edit Sale</h2>
</div>
...
$vat = $item->tax_applied ?? ($amount * 0.15);
@endphp
<tr class="line-item border-b border-slate-200 dark:border-slate-700">
<td class="px-4 py-3">
<select class="product-select w-full h-10 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 text-slate-900 dark:text-white" name="items[{{ $idx }}][product_id]" required>
...
<td class="px-4 py-3">
<input class="price-input w-full h-10 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 text-right text-slate-900 dark:text-white" name="items[{{ $idx }}][selling_price]" type="number" step="0.01" min="0" value="{{ old("items.{$idx}.selling_price", $item->selling_price) }}" required>
</td>
<td class="px-4 py-3">
<input class="qty-input w-full h-10 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 text-right text-slate-900 dark:text-white" name="items[{{ $idx }}][quantity]" type="number" min="1" value="{{ old("items.{$idx}.quantity", $item->quantity) }}" required>
</td>
...
<tr class="line-item border-b border-slate-200 dark:border-slate-700">
<td class="px-4 py-3">
<select class="product-select w-full h-10 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 text-slate-900 dark:text-white" name="items[{{ $sale->items->count() }}][product_id]">
<option value="">Select product</option>
@foreach($products as $p)
...
<td class="px-4 py-3">
<input class="price-input w-full h-10 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 text-right text-slate-900 dark:text-white" name="items[{{ $sale->items->count() }}][selling_price]" type="number" step="0.01" min="0" value="0">
</td>
<td class="px-4 py-3">
<input class="qty-input w-full h-10 rounded-lg border border-slate-200 dark:border-slate-600 bg-white dark:bg-slate-800 px-3 text-right text-slate-900 dark:text-white" name="items[{{ $sale->items->count() }}][quantity]" type="number" min="1" value="1">
</td>
<td class="px-4 py-3 text-right">
...
<p class="text-lg font-bold text-slate-900 dark:text-white tabular-nums">Total: {{ $currencySymbol ?? '$' }} <span id="total-display">0.00</span></p>
</div>
</div>
</div>
...
(function() {
    ...
    document.getElementById('add-row')?.addEventListener('click', function() {
        ...
        newRow.querySelector('.product-select').removeAttribute('required');
