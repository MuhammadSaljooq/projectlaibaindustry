...
/**
 * Dompdf calls realpath() on this value when the PDF service boots (see barryvdh/laravel-dompdf).
 * If `public/` is missing from your deployed tree or lives outside the app root, the default
 * `realpath(base_path('public'))` fails with "Cannot resolve public path".
 *
 * Set DOMPDF_PUBLIC_PATH in .env to an absolute path to your web public directory when needed.
 */
$dompdfPublicPath = env('DOMPDF_PUBLIC_PATH');
if (is_string($dompdfPublicPath) && $dompdfPublicPath !== '') {
    $resolved = realpath($dompdfPublicPath);
    $dompdfPublicPath = $resolved !== false ? $resolved : $dompdfPublicPath;
} else {
    $dompdfPublicPath = null;
    foreach ([public_path(), base_path('public'), base_path(), storage_path('app')] as $candidate) {
        $resolved = realpath($candidate);
        if ($resolved !== false && is_dir($resolved)) {
            $dompdfPublicPath = $resolved;
            break;
        }
    }
    if ($dompdfPublicPath === null) {
        foreach ([storage_path('app'), sys_get_temp_dir()] as $fallback) {
    ...
    if ($dompdfPublicPath === null) {
        $dompdfPublicPath = rtrim(sys_get_temp_dir(), DIRECTORY_SEPARATOR);
    }
}
